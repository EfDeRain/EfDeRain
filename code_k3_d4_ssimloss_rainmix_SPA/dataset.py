...
class DenoisingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ## read an image
        #img_rainy = np.array(Image.open(self.imglist[index][0]))
        #img_gt = np.array(Image.open(self.imglist[index][1]))
        
        img_rainy = cv2.imread(self.imglist[index][0])
        img_gt = cv2.imread(self.imglist[index][1])
        while ((img_rainy is None) or (img_gt is None)):
            print(self.imglist[index][0])
            print(self.imglist[index][1])
            index_new = random.randint(0, len(self.imglist)-1)
            img_rainy = cv2.imread(self.imglist[index_new][0])
            img_gt = cv2.imread(self.imglist[index_new][1])

        img_rainy = cv2.cvtColor(img_rainy, cv2.COLOR_BGR2RGB)
        img_gt = cv2.cvtColor(img_gt, cv2.COLOR_BGR2RGB)
        
        '''
        print("rainy",self.imglist[index][0])
        print("gt",self.imglist[index][1])
        print("!")
        print(img_rainy.shape, img_gt.shape)
        '''
        '''
        try:
            print(img_rainy.shape, img_gt.shape)
