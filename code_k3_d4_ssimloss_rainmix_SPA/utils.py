...
def get_files(path):
    ...
    if "Training" in path:
        '''
        path_txt = path + "/real_world.txt"
        f = open(path_txt)
        line = f.readline()
        while line:
            line = line[:-1]
            file_rainy, file_gt = line.split(' ')
            ret.append([path + file_rainy, path + file_gt])
            line = f.readline()
        f.close()


        '''
        path_rainy = path + "/real_world"
        path_gt = path + "/real_world_gt"

        for root, dirs, files in os.walk(path_rainy):
            files.sort()

            for name in files:
                if (name.split('.')[1] != "png") or ("resize" in name):
                    continue
                part1234 = name.split('.')[0]
                part1, part234 = part1234.split('-')
                part2, part3, part4 = part234.split('_') 

                file_rainy = root + "/" + name
                file_gt = path_gt + "/" + part1 + "/" + part1 + "_" + part3 + "_" + part4 + ".png"
